    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-30 backdrop-blur-md bg-background/70 border-b border-border/40">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
          <a href="#top" className="flex items-center gap-2">
            ...
            <span className="font-display text-2xl tracking-wide">QT <span className="text-gold">Designs</span></span>
          </a>
          ...
          <button onClick={() => setOrderOpen(true)} className="rounded-full bg-gold px-5 py-2 text-sm font-medium text-primary-foreground transition hover:opacity-90">
            Order Now
          </button>
        </div>
      </header>
      ...
      <section id="top" className="relative overflow-hidden">
        <div className="mx-auto grid max-w-7xl gap-12 px-6 py-20 md:grid-cols-2 md:items-center md:py-32">
          <div className="relative z-10">
            <p className="mb-6 inline-flex items-center gap-2 rounded-full border border-gold/30 bg-gold/5 px-4 py-1.5 text-xs uppercase tracking-[0.2em] text-gold">
              <Crescent className="h-3 w-3" /> Bespoke Eid Gifting
            </p>
            <h1 className="font-display text-5xl leading-[1.05] md:text-7xl">
              Stickers, favours <br/>& keepsakes <em className="text-gold not-italic">made with love.</em>
            </h1>
            ...
            <div className="mt-10 flex flex-wrap gap-4">
              <button onClick={() => setOrderOpen(true)} className="group relative overflow-hidden rounded-full bg-gold px-8 py-4 font-medium text-primary-foreground shadow-[var(--shadow-elegant)] transition hover:scale-[1.02]">
                Place an order →
              </button>
            ...
            <div className="mt-12 flex gap-10 text-sm">
              <div><div className="font-display text-3xl text-gold">500+</div><div className="text-muted-foreground">Happy gifters</div></div>
              <div><div className="font-display text-3xl text-gold">7 days</div><div className="text-muted-foreground">Turnaround</div></div>
              <div><div className="font-display text-3xl text-gold">100%</div><div className="text-muted-foreground">Handmade</div></div>
            </div>
          </div>

          <div className="relative">
            <div className="absolute -inset-8 rounded-[3rem] bg-gold/10 blur-3xl" aria-hidden />
            <div className="relative overflow-hidden rounded-[2rem] border border-gold/20 shadow-[var(--shadow-card)]">
            ...
            <div className="absolute -bottom-6 -left-6 hidden rounded-2xl border border-gold/30 bg-card px-5 py-4 shadow-[var(--shadow-card)] md:block">
              <div className="flex items-center gap-3">
                <Crescent className="h-6 w-6 text-gold" />
                <div>
      ...
      <section id="collection" className="border-t border-border/40 py-24">
        <div className="mx-auto max-w-7xl px-6">
          <div className="mb-14 flex items-end justify-between gap-6">
            <div>
              <p className="mb-3 text-xs uppercase tracking-[0.2em] text-gold">The Collection</p>
              ...
              <article key={p.name} className="group">
                <div className="relative aspect-[4/5] overflow-hidden rounded-2xl border border-border bg-secondary">
                  ...
                  <div className="absolute inset-x-3 bottom-3 flex items-center justify-between rounded-full bg-background/85 px-4 py-2 text-xs backdrop-blur">
                    ...
                    <button onClick={() => setOrderOpen(true)} className="text-foreground hover:text-gold">Order →</button>
                  </div>
                </div>
      ...
      <section id="story" className="relative border-t border-border/40 py-24">
        <div className="mx-auto grid max-w-7xl gap-16 px-6 md:grid-cols-2 md:items-center">
          <div>
            <p className="mb-3 text-xs uppercase tracking-[0.2em] text-gold">Our Story</p>
            <h2 className="font-display text-4xl md:text-5xl leading-tight">Small details, <em className="text-gold not-italic">big moments.</em></h2>
            ...
            <ul className="mt-8 space-y-3 text-sm">
              {["Personalized to every guest","Premium paper & gold foil","Made-to-order, never mass produced","Ships across South Africa"].map(t => (
                <li key={t} className="flex items-center gap-3"><Crescent className="h-4 w-4 text-gold" />{t}</li>
              ))}
            </ul>
          </div>
          <div className="relative">
            <div className="rounded-2xl border border-gold/20 bg-card p-8 shadow-[var(--shadow-card)]">
      ...
      <section id="contact" className="border-t border-border/40 py-24">
        <div className="mx-auto max-w-4xl px-6 text-center">
          <Crescent className="mx-auto h-10 w-10 text-gold" />
          ...
          <button onClick={() => setOrderOpen(true)} className="mt-10 rounded-full bg-gold px-10 py-4 font-medium text-primary-foreground shadow-[var(--shadow-elegant)] transition hover:scale-[1.02]">
            Start your order
          </button>
        </div>
      </section>
      ...
      <footer className="border-t border-border/40 py-10">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-6 text-sm text-muted-foreground md:flex-row">
          <div className="flex items-center gap-2"><Crescent className="h-4 w-4 text-gold" /> QT Designs © {new Date().getFullYear()}</div>
...
function OrderDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="relative w-full max-w-lg rounded-2xl border border-gold/20 bg-card p-8 shadow-[var(--shadow-elegant)]" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute right-4 top-4 text-muted-foreground hover:text-gold" aria-label="Close">✕</button>
        {sent ? (
          <div className="py-8 text-center">
            ...
            <button onClick={onClose} className="mt-6 rounded-full bg-gold px-6 py-2.5 text-primary-foreground">Close</button>
          </div>
        ) : (
          <>
            <p className="text-xs uppercase tracking-[0.2em] text-gold">Place Order</p>
            <h3 className="mt-2 font-display text-3xl">Tell us about your gift</h3>
            <form onSubmit={submit} className="mt-6 space-y-4">
              <Field label="Your name"><input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full rounded-lg border border-border bg-background px-4 py-2.5 outline-none focus:border-gold" /></Field>
              <Field label="Item"><select value={form.item} onChange={(e) => setForm({ ...form, item: e.target.value })} className="w-full rounded-lg border border-border bg-background px-4 py-2.5 outline-none focus:border-gold">
                {products.map(p => <option key={p.name}>{p.name}</option>)}
              </select></Field>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Quantity"><input type="number" min="1" value={form.qty} onChange={(e) => setForm({ ...form, qty: e.target.value })} className="w-full rounded-lg border border-border bg-background px-4 py-2.5 outline-none focus:border-gold" /></Field>
                <Field label="Needed by"><input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="w-full rounded-lg border border-border bg-background px-4 py-2.5 outline-none focus:border-gold" /></Field>
              </div>
              <Field label="Notes (names, colours, theme)"><textarea rows={3} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="w-full rounded-lg border border-border bg-background px-4 py-2.5 outline-none focus:border-gold" /></Field>
              <button type="submit" className="w-full rounded-full bg-gold py-3 font-medium text-primary-foreground transition hover:opacity-90">
